...
"""Manages the AWS config namespace.
...
Possible actions: [list|activate|cleanup]
    ...
    Activate:
        - Sets a project configuration to be "active". This means that all commands invoked from CLI
            will use the given project, region, etc. associated with the active config.
...
Examples:
    ...
    axlearn aws config list

    # List available projects by label.
    axlearn aws config list --label=my-label

    # Activate a project by prompt.
    axlearn aws config activate

    # Activate a project by one or more labels.
    axlearn aws config activate --label=my-label --label=my-other-label
...
def aws_settings(
    ...
    region = flag_values.get("region", None)
    if project and region:
        config_name = _project_config_key(project, region)
    else:
    ...
    if required and not project_configs:
        ...
        logging.error(
            "Unknown settings for project=%s and region=%s; "
...
def _project_config_key(project: str, region: str) -> str:
    """Constructs a toml-friendly name uniquely identified by project, aws-region."""
